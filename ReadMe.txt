================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: robot_arm_head プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した robot_arm_head には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには robot_arm_head アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

robot_arm_head.vcxproj
...
robot_arm_head.vcxproj.filters
...
robot_arm_head.h
   ...
   Crobot_arm_headApp アプリケーション クラスを宣言します。

robot_arm_head.cpp
   これは、アプリケーション クラス Crobot_arm_headApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\robot_arm_head.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル robot_arm_head.rc に含まれていま
   す。

res\robot_arm_head.rc2
...
robot_arm_headDlg.h, robot_arm_headDlg.cpp - ダイアログ
   これらのファイルは Crobot_arm_headDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な robot_arm_head.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   robot_arm_head.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
